 ...
 * ********************* 配置项 *********************
 * oms.storage.dfs.xugu_series.driver
 * oms.storage.dfs.xugu_series.url
 * oms.storage.dfs.xugu_series.username
 * oms.storage.dfs.xugu_series.password
 * oms.storage.dfs.xugu_series.auto_create_table
 * oms.storage.dfs.xugu_series.table_name
 */
@Slf4j
@Priority(value = Integer.MAX_VALUE - 2)
@Conditional(XuGuSeriesDfsService.XuGuSeriesCondition.class)
public class XuGuSeriesDfsService extends AbstractDFsService {

    private DataSource dataSource;

    private static final String TYPE_XUGU = "xugu_series";

    /**
     * 数据库驱动，XuguDB 12.0.0 为 com.xugu.cloudjdbc.Driver
     */
    private static final String KEY_DRIVER_NAME = "driver";
    /**
     * 数据库地址，比如 jdbc:xugu://localhost:5138/powerjob-daily
     */
    ...
    private void executeDelete(String sql) {
        try (Connection con = dataSource.getConnection()) {
            ...
            log.error("[XuGuSeriesDfsService] executeDelete failed, sql: {}", sql);
        }
    }
    ...
    public void store(StoreRequest storeRequest) throws IOException {
        ...
        try (Connection con = dataSource.getConnection()) {
            ...
            log.info("[XuGuSeriesDfsService] store [{}] successfully, cost: {}", fileLocation, sw);

        } catch (Exception e) {
            log.error("[XuGuSeriesDfsService] store [{}] failed!", fileLocation);
    ...
    public void download(DownloadRequest downloadRequest) throws IOException {
        ...
        try (Connection con = dataSource.getConnection()) {
            ...
            if (!exist) {
                log.warn("[XuGuSeriesDfsService] download file[{}] failed due to not exits!", fileLocation);
                return;
            }
            ...
            log.info("[XuGuSeriesDfsService] download [{}] successfully, cost: {}", fileLocation, sw);

        } catch (Exception e) {
            log.error("[XuGuSeriesDfsService] download file [{}] failed!", fileLocation, e);
            ExceptionUtils.rethrow(e);
        }
    ...
    public Optional<FileMeta> fetchFileMeta(FileLocation fileLocation) throws IOException {
        ...
        try (Connection con = dataSource.getConnection()) {
            ...
            log.error("[XuGuSeriesDfsService] fetchFileMeta [{}] failed!", fileLocation);
            ExceptionUtils.rethrow(e);
        }
    ...
    public void cleanExpiredFiles(String bucket, int days) {
        ...
        log.info("[XuGuSeriesDfsService] start to cleanExpiredFiles, targetDeleteTime: {}", targetDeleteTime);
        String fSQL = dSQLPrefix.concat(String.format(" where gmt_modified < '%s'", targetDeleteTime));
        log.info("[XuGuSeriesDfsService] cleanExpiredFiles SQL: {}", fSQL);
        executeDelete(fSQL);
    }
    ...
    protected void init(ApplicationContext applicationContext) {
        ...
        XuGuProperty xuguProperty = new XuGuProperty()
                .setDriver(fetchProperty(env, TYPE_XUGU, KEY_DRIVER_NAME))
                .setUrl(fetchProperty(env, TYPE_XUGU, KEY_URL))
                .setUsername(fetchProperty(env, TYPE_XUGU, KEY_USERNAME))
                .setPassword(fetchProperty(env, TYPE_XUGU, KEY_PASSWORD))
                .setAutoCreateTable(Boolean.TRUE.toString().equalsIgnoreCase(fetchProperty(env, TYPE_XUGU, KEY_AUTO_CREATE_TABLE)));

        try {
            initDatabase(xuguProperty);
            initTable(xuguProperty);
        } catch (Exception e) {
            log.error("[XuGuSeriesDfsService] init datasource failed!", e);
            ExceptionUtils.rethrow(e);
        }

        log.info("[XuGuSeriesDfsService] initialize successfully, THIS_WILL_BE_THE_STORAGE_LAYER.");
    }

    void initDatabase(XuGuProperty property) {

        log.info("[XuGuSeriesDfsService] init datasource by config: {}", property);
    ...
    void initTable(XuGuProperty property) throws Exception {

        if (property.autoCreateTable) {

            String createTableSQL = fullSQL(CREATE_TABLE_SQL);

            log.info("[XuGuSeriesDfsService] use create table SQL: {}", createTableSQL);
            try (Connection connection = dataSource.getConnection()) {
                connection.createStatement().execute(createTableSQL);
                log.info("[XuGuSeriesDfsService] auto create table successfully!");
            }
        }
    }
    ...
    private String parseTableName() {
        ...
        String tableName = fetchProperty(applicationContext.getEnvironment(), TYPE_XUGU, KEY_TABLE_NAME);
    ...
    public static class XuGuSeriesCondition extends PropertyAndOneBeanCondition {
        @Override
        protected List<String> anyConfigKey() {
            return Lists.newArrayList("oms.storage.dfs.xugu_series.url");
        }
